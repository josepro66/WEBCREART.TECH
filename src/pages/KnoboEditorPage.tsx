/**
 * KnoboEditorPage.tsx — Editor MIDI del KNOBO (8 knobs) con vista 3D.
 *
 * Soporta 3 bancos (A/B/C), 5 comportamientos, config de CC/canal/rango
 * por knob. Integrable al ecosistema unificado vía portales.
 */

import React, { useState, useCallback, useEffect, useRef } from 'react'
import { createPortal } from 'react-dom'
import StarfieldBackground from '../configurator/components/StarfieldBackground'
import KnoboScene, { type KnoboSceneHandle } from '../knobo-editor/KnoboScene'
import {
  type Bank,
  type KnoboControlId,
  type KnoboControlState,
  type KnoboControlConfig,
  BANKS,
  makeKnoboInitialState,
  knoboLabelFor,
  ALL_KNOBO_CONTROL_IDS,
  KNOBO_CC_BY_BANK,
  KNOBO_CHANNEL,
  KNOBO_BANK_CC,
  KNOBO_TOUCH_SELECT_CC,
  KNOBO_BEHAVIORS,
  KNOBO_PARAM_LABELS,
  KNOBO_BEHAVIOR_IDS,
} from '../knobo-editor/knoboState'
...
const P = {
  ...
  sub: '#7A7870',
  accent: '#FF3D77',
  accentSoft: 'rgba(255,61,119,0.15)',
}

const MANUFACTURER_ID = 0x7d
const DEVICE_ID_KNOBO = 0x12
...
const N_BANKS_FW = 3
const N_KNOBS_FW = 8
const PRESET_BINARY_SIZE = 3 + N_BANKS_FW * N_KNOBS_FW * 6 + 1
const PRESET_MAGIC = 0xfa
const FORMAT_VERSION = 1

interface EngineEntry { value: number; target: number; phase: number }

function makeEngines(): Record<KnoboControlId, Record<Bank, EngineEntry>> {
  const out = {} as Record<KnoboControlId, Record<Bank, EngineEntry>>
  for (const id of ALL_KNOBO_CONTROL_IDS) {
...
const KnoboEditorPage: React.FC<Props> = ({
  embedded, embeddedTitle, embeddedTint, sceneSlot, panelSlot, active, onActivate,
}) => {
  const portalMode = typeof onActivate === 'function'
  const [midiState, setMidiState] = useState<KnoboControlState>(makeKnoboInitialState)
  const [bank, setBank] = useState<Bank>('A')
  const [selected, setSelected] = useState<KnoboControlId | null>(null)
  const [connected, setConnected] = useState(false)
  const [statusText, setStatusText] = useState('Buscando Knobo...')
  const sceneRef = useRef<KnoboSceneHandle>(null)
  ...
  const selectCtrl = (id: KnoboControlId) => { setSelected(id); onActivate?.() }

  const log = useCallback((text: string) => {
    setLogEntries((prev) => { const next = [...prev, text]; return next.length > 60 ? next.slice(-60) : next })
  }, [])

  useEffect(() => { if (logRef.current) logRef.current.scrollTop = logRef.current.scrollHeight }, [logEntries])

  const sendMidiFromBank = useCallback((id: KnoboControlId, b: Bank, rawValue: number) => {
      ...
      for (const id of ALL_KNOBO_CONTROL_IDS) {
        for (const b of BANKS) {
          ...
          if (outVal !== null) {
            ...
            if (b === bankRef.current) sceneRef.current?.setKnobValue(id, outVal)
          }
        }
      }
  ...
  const processIncoming = useCallback((d1: number, d2: number) => {
    ...
    if (d1 === KNOBO_BANK_CC.A || d1 === KNOBO_BANK_CC.B || d1 === KNOBO_BANK_CC.C) {
      if (d2 > 0) {
        const newBank: Bank = d1 === KNOBO_BANK_CC.A ? 'A' : d1 === KNOBO_BANK_CC.B ? 'B' : 'C'
        if (newBank !== currentBank) { setBank(newBank); log(`Banco cambiado a ${newBank}`) }
      }
      return
    }

    if (d1 === KNOBO_TOUCH_SELECT_CC) {
      if (d2 >= 0 && d2 < 8) {
        const id = ALL_KNOBO_CONTROL_IDS[d2]
        setSelected(id)
        log(`Touch select: ${knoboLabelFor(id)}`)
      }
      return
    }
    ...
    for (let i = 0; i < ALL_KNOBO_CONTROL_IDS.length && matchIdx === -1; i++) {
      const kid = ALL_KNOBO_CONTROL_IDS[i]
      for (const b of BANKS) {
        const st = s[kid][b]
        if (st.type === 'cc' && st.num === d1) { matchIdx = i; matchBank = b; break }
      }
    }

    if (matchIdx === -1 || !matchBank) return

    const id = ALL_KNOBO_CONTROL_IDS[matchIdx]
    ...
    let outVal = d2
    switch (cfg.behavior) {
      case 'espejo': outVal = 127 - d2; break
      case 'inercia': eng.target = d2; break
      case 'random': outVal = Math.max(0, Math.min(127, d2 + (Math.random() - 0.5) * 2 * cfg.param)); break
      case 'oscila': eng.target = d2; break
    }

    if (!isCurrentBank) { setBank(matchBank); log(`Auto-switch banco → ${matchBank}`) }
    const autonomous = cfg.behavior === 'oscila' || cfg.behavior === 'random'
    if (!autonomous) sceneRef.current?.setKnobValue(id, outVal)
    if (cfg.behavior !== 'oscila') setSelected(id)
    sendMidiFromBank(id, matchBank, outVal)
    log(`${knoboLabelFor(id)} (${cfg.behavior}) banco${matchBank} → CC${cfg.num} val=${Math.round(outVal)}`)
  ...
  function sendSysex(cmd: number, payload: number[]) {
    const out = midiOutputRef.current
    if (!out) return false
    out.send([0xf0, MANUFACTURER_ID, DEVICE_ID_KNOBO, cmd, ...payload, 0xf7])
    return true
  }

  function serializePreset(): number[] {
    const s = midiStateRef.current
    const bytes: number[] = [PRESET_MAGIC, FORMAT_VERSION, 0x00]
    for (const b of BANKS) {
      for (let i = 0; i < N_KNOBS_FW; i++) {
        const id = ALL_KNOBO_CONTROL_IDS[i]
        const c = s[id][b]
        const paramMax = c.behavior === 'random' ? 127 : 100
        bytes.push(c.num & 0x7f)
        bytes.push(packControlByte(c.type === 'cc' ? 1 : 0, (c.chan - 1) & 0x0f, 0))
        bytes.push(KNOBO_BEHAVIOR_IDS[c.behavior] ?? 0)
  ...
  const saveToDevice = useCallback(async () => {
    if (!midiOutputRef.current || !midiInputRef.current) {
      setDeviceSaveStatus('Conecta el Knobo (entrada y salida MIDI) antes de guardar.')
      return
    }
    let presetBytes: number[]
    try { presetBytes = serializePreset() } catch (e: any) { setDeviceSaveStatus('Error: ' + e.message); return }
    setDeviceSaveProgress(0)
    setDeviceSaveStatus('Enviando configuracion al Knobo...')
    ...
    if (resp.command === CMD_WRITE_OK) {
      setDeviceSaveStatus('Guardado en el Knobo.')
      log('Preset escrito en EEPROM.')
    } else {
    ...
    if ((window as any).showSaveFilePicker) {
      try {
        const handle = await (window as any).showSaveFilePicker({
          suggestedName: 'knobo_config.knobo',
          types: [{ description: 'Config Knobo', accept: { 'application/json': ['.knobo', '.json'] } }],
        })
        const w = await handle.createWritable(); await w.write(data); await w.close()
        log('Config guardada: ' + handle.name)
      } catch (e: any) { if (e.name !== 'AbortError') log('Error: ' + e.message) }
    } else {
      const a = document.createElement('a')
      a.href = URL.createObjectURL(new Blob([data], { type: 'application/json' }))
      a.download = 'knobo_config.knobo'; a.click()
    }
  }, [log])
  ...
  const loadConfig = useCallback(async () => {
    const applyFull = (cfg: any) => {
      ...
      setMidiState((prev) => {
        ...
        Object.keys(cfg.state).forEach((id) => {
          if (next[id as KnoboControlId]) {
            next[id as KnoboControlId] = { ...next[id as KnoboControlId] }
            BANKS.forEach((b) => { if (cfg.state[id][b]) next[id as KnoboControlId][b] = { ...next[id as KnoboControlId][b], ...cfg.state[id][b] } })
          }
        })
        return next
      })
    ...
    if ((window as any).showOpenFilePicker) {
      try {
        const [handle] = await (window as any).showOpenFilePicker({ types: [{ description: 'Config Knobo', accept: { 'application/json': ['.knobo', '.json'] } }] })
        applyFull(JSON.parse(await (await handle.getFile()).text()))
      } catch (e: any) { if (e.name !== 'AbortError') log('Error: ' + e.message) }
    } else {
      const input = document.createElement('input'); input.type = 'file'; input.accept = '.knobo,.json'
      input.onchange = async () => { try { applyFull(JSON.parse(await input.files![0].text())) } catch (e: any) { log('Error: ' + e.message) } }
      input.click()
    }
  }, [log])

  const handleFactoryReset = useCallback(() => {
    if (!window.confirm('Restablecer valores de fabrica?\nEsta accion no se puede deshacer.')) return
    const out = midiOutputRef.current
    if (out) { for (let ch = 0; ch < 16; ch++) { out.send([0xb0 | ch, 123, 0]); out.send([0xb0 | ch, 120, 0]) } }
    try { localStorage.removeItem('knobo-state') } catch {}
    setMidiState(makeKnoboInitialState())
    setBank('A'); setSelected(null); setDeviceSaveStatus(''); setDeviceSaveProgress(null)
    log('Valores de fabrica restablecidos.')
  }, [log])

  const updateCfg = useCallback((id: KnoboControlId, b: Bank, patch: Partial<KnoboControlConfig>) => {
  ...
  const isKnoboDevice = (name: string | null | undefined) => /knobo/i.test(name || '')
  ...
  const setupMidiPorts = useCallback((access: MIDIAccess) => {
    ...
    const firstIn = allInputs.find((p) => isKnoboDevice(p.name)) || allInputs[0]
    if (firstIn && firstIn !== midiInputRef.current) {
      ...
      firstIn.onmidimessage = (msg: MIDIMessageEvent) => {
        ...
        if (data[0] === 0xf0) {
          if (data.length >= 4 && data[1] === MANUFACTURER_ID && data[2] === DEVICE_ID_KNOBO) {
            const command = data[3]; const payload = data.slice(4, data.length - 1)
            if (pendingResponseRef.current) {
              const resolve = pendingResponseRef.current; pendingResponseRef.current = null
        ...
        if ((statusByte & 0xf0) === 0xb0) { onActivateRef.current?.(); processIncoming(d1, d2) }
      }
      setConnected(true); setStatusText(firstIn.name || 'MIDI Conectado'); log('Conectado a: ' + firstIn.name)
    } else if (!firstIn) { midiInputRef.current = null; setConnected(false); setStatusText('Knobo no detectado.') }
    setOutputList(allOutputs.map((o, idx) => ({ idx, name: o.name })))
    if (allOutputs.length > 0 && midiOutputRef.current === null) {
      const kIdx = allOutputs.findIndex((p) => isKnoboDevice(p.name))
      const useIdx = kIdx >= 0 ? kIdx : 0
      midiOutputRef.current = allOutputs[useIdx]; setSelectedOutputIdx(useIdx)
    }
  }, [processIncoming, log])

  useEffect(() => {
    const nav = navigator as any
    if (!nav.requestMIDIAccess) { setStatusText('Web MIDI no soportado'); return }
    nav.requestMIDIAccess({ sysex: true })
      .then((access: MIDIAccess) => { midiAccessRef.current = access; setupMidiPorts(access); access.addEventListener('statechange', () => setupMidiPorts(access)) })
      .catch((err: Error) => { setStatusText('Error MIDI: ' + err.message); log('Error: ' + err.message) })
  }, [setupMidiPorts, log])
  ...
  useEffect(() => {
    const id = setInterval(() => { try { localStorage.setItem('knobo-state', JSON.stringify(midiStateRef.current)) } catch {} }, 2000)
    return () => clearInterval(id)
  }, [])

  useEffect(() => {
    try {
      const saved = localStorage.getItem('knobo-state')
      if (saved) {
        ...
        setMidiState((prev) => {
          ...
          Object.keys(parsed).forEach((id) => {
            if (next[id as KnoboControlId]) {
              next[id as KnoboControlId] = { ...next[id as KnoboControlId] }
              BANKS.forEach((b) => { if (parsed[id]?.[b]) next[id as KnoboControlId][b] = { ...next[id as KnoboControlId][b], ...parsed[id][b] } })
            }
          })
          return next
        })
  ...
  const sceneEl = <KnoboScene ref={sceneRef} selectedId={selected} onSelect={selectCtrl} showLabels={showLabels} />
  const cfg = selected ? midiState[selected][bank] : null
  const selectedBehavior = cfg ? KNOBO_BEHAVIORS.find((b) => b.id === cfg.behavior) : null
  const paramLabel = cfg ? KNOBO_PARAM_LABELS[cfg.behavior] : null
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, color: P.text, fontFamily: 'Inter, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderBottom: `1px solid ${P.border}`, background: 'rgba(14,14,16,0.6)', flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setShowLabels((v) => !v)}
          style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, fontWeight: 700, background: showLabels ? 'rgba(57,255,20,0.15)' : '#1A1A20', color: showLabels ? '#39ff14' : P.sub, border: `1px solid ${showLabels ? '#39ff14' : P.border}`, borderRadius: 7, padding: '5px 8px', cursor: 'pointer' }}>MAP</button>
        <button onClick={() => setLogOpen((v) => !v)}
          style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, background: logOpen ? P.accentSoft : '#1A1A20', color: logOpen ? tint : P.sub, border: `1px solid ${logOpen ? tint : P.border}`, borderRadius: 7, padding: '6px 10px', cursor: 'pointer' }}>Consola</button>
        <div style={{ display: 'flex', gap: 5, marginLeft: 'auto' }}>
          {BANKS.map((b) => (
            <button key={b} onClick={() => setBank(b)}
              style={{ width: 32, height: 32, borderRadius: 7, border: `1px solid ${b === bank ? tint : P.border}`, background: b === bank ? tint : '#1A1A20', color: b === bank ? '#0E0E10' : P.text, fontWeight: 700, fontFamily: 'JetBrains Mono, monospace', fontSize: 12, cursor: 'pointer' }}>{b}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 18 }}>
        {!selected ? (
          <div style={{ display: 'grid', justifyItems: 'center', gap: 12, textAlign: 'center', marginTop: 56 }}>
            ...
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, fontWeight: 700, letterSpacing: '0.14em', color: '#D8DBE0', textTransform: 'uppercase' }}>Selecciona un knob</div>
            <div style={{ color: '#8A919E', fontSize: 12.5, lineHeight: 1.6, maxWidth: 260 }}>Haz clic en un knob del modelo 3D — o muévelo en el hardware — para configurar su ruteo MIDI.</div>
          </div>
        ) : cfg && (
          <div>
            ...
            <h2 style={{ fontFamily: "'Orbitron', sans-serif", fontSize: 22, fontWeight: 800, letterSpacing: '0.06em', margin: '4px 0 18px', color: P.text }}>{knoboLabelFor(selected)}</h2>

            <Field label="Etiqueta">
              <input type="text" value={cfg.label} onChange={(e) => updateCfg(selected, bank, { label: e.target.value })} maxLength={20} style={inputStyle} placeholder="ej: Cutoff" />
            </Field>
            <Field label="Tipo MIDI">
              <select value={cfg.type} onChange={(e) => updateCfg(selected, bank, { type: e.target.value as 'cc' | 'pb' })} style={inputStyle}>
                <option value="cc">CC (Control Change)</option>
                <option value="pb">PB (Pitch Bend)</option>
              </select>
            </Field>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {cfg.type === 'cc' && <Field label="CC #"><input type="number" min={0} max={127} value={cfg.num} onChange={(e) => updateCfg(selected, bank, { num: clamp(Number(e.target.value), 0, 127) })} style={inputStyle} /></Field>}
            ...
            <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, letterSpacing: '0.12em', color: P.sub, textTransform: 'uppercase', margin: '14px 0 6px' }}>Rango de salida</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <Field label="Min"><input type="number" min={0} max={127} value={cfg.min} onChange={(e) => updateCfg(selected, bank, { min: clamp(Number(e.target.value), 0, 127) })} style={inputStyle} /></Field>
              <Field label="Max"><input type="number" min={0} max={127} value={cfg.max} onChange={(e) => updateCfg(selected, bank, { max: clamp(Number(e.target.value), 0, 127) })} style={inputStyle} /></Field>
            </div>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
              {KNOBO_BEHAVIORS.map((bh) => {
                const act = cfg.behavior === bh.id
                return (
                  <button key={bh.id} onClick={() => updateCfg(selected, bank, { behavior: bh.id })} title={bh.info.replace(/\n/g, ' ')}
                    style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, fontWeight: act ? 700 : 400, background: act ? P.accentSoft : '#1A1A20', color: act ? tint : P.sub, border: `1px solid ${act ? tint : P.border}`, borderRadius: 7, padding: '6px 10px', cursor: 'pointer', letterSpacing: '0.04em' }}>
                    {bh.icon} {bh.name}
                  </button>
                )
              })}
            </div>
            {selectedBehavior && <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: P.sub, lineHeight: 1.5, margin: '0 0 10px', whiteSpace: 'pre-line' }}>{selectedBehavior.desc}</p>}
            {paramLabel && (
              <Field label={paramLabel}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <input type="range" min={0} max={100} value={cfg.param} onChange={(e) => updateCfg(selected, bank, { param: Number(e.target.value) })} style={{ flex: 1, accentColor: tint }} />
                  <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: P.sub, width: 32, textAlign: 'right' }}>{cfg.param}</span>
                </div>
              </Field>
            )}

            <button onClick={() => sceneRef.current?.setKnobValue(selected, 64)}
              style={{ marginTop: 14, width: '100%', padding: '10px 14px', background: P.accentSoft, border: `1px solid ${tint}`, color: tint, fontFamily: 'JetBrains Mono, monospace', fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase', borderRadius: 7, cursor: 'pointer' }}>
              Probar &rarr; mover knob al centro
            </button>
            ...
            {deviceSaveStatus && <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: P.sub, marginTop: 10, lineHeight: 1.5 }}>{deviceSaveStatus}</p>}
  ...
  const consoleEl = logOpen && (
    <div style={{ position: 'fixed', bottom: 20, right: 20, width: 340, maxHeight: 280, background: '#18181B', border: `1px solid ${P.border}`, borderRadius: 14, display: 'flex', flexDirection: 'column', zIndex: 200, boxShadow: '0 8px 32px rgba(0,0,0,0.6)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderBottom: `1px solid ${P.border}`, fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: P.sub }}>
        <span>Consola MIDI &middot; Knobo</span>
  ...
  return (
    <div style={{ ...(embedded ? { position: 'relative', width: '100%', height: '100%', display: 'grid' } : { position: 'fixed', inset: 0, display: 'grid' }), color: P.text, fontFamily: 'Inter, sans-serif', gridTemplateColumns: '1.4fr 1fr', background: P.bg, minHeight: 0 }}>
      <StarfieldBackground />
      <div style={{ position: 'relative', minHeight: 0, minWidth: 0, background: 'transparent' }}>{sceneEl}</div>
...
const Field: React.FC<{ label: string; children: React.ReactNode }> = ({ label, children }) => (
  <div style={{ marginBottom: 10 }}>
    <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, letterSpacing: '0.08em', color: '#7A7870', textTransform: 'uppercase', marginBottom: 4 }}>{label}</div>
    {children}
  </div>
)

const inputStyle: React.CSSProperties = {
  width: '100%', background: '#1A1A20', border: '1px solid #2A2A32', borderRadius: 6,
  padding: '7px 9px', color: '#E8E6E0', fontFamily: 'JetBrains Mono, monospace', fontSize: 12, boxSizing: 'border-box',
}

function actionBtnStyle(tint: string): React.CSSProperties {
  return { fontFamily: 'JetBrains Mono, monospace', fontSize: 10, letterSpacing: '0.06em', textTransform: 'uppercase', background: 'rgba(255,61,119,0.08)', border: `1px solid ${tint}`, color: tint, borderRadius: 7, padding: '8px 12px', cursor: 'pointer' }
}

function clamp(n: number, lo: number, hi: number): number { return Number.isNaN(n) ? lo : Math.max(lo, Math.min(hi, n)) }

export default KnoboEditorPage
